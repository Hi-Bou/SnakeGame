...
class SNAKE:
    ...
    def move_snake(self):
        if self.new_block == True and main_game.game_started == True:
            body_copy = self.body[:]
            body_copy.insert(0, body_copy[0] + self.direction)
            self.body = body_copy[:]
            self.new_block = False
        elif main_game.game_started == True:
...
class MAIN:
    ...
    def check_collision(self):
        if self.fruit.pos == self.snake.body[0]:
            ...
            self.play_sound(self.snake.crunch_sound)
    ...
    def game_over_method(self):
        ...
        if self.sound_playing is False:
            self.play_sound(self.seeitcoming) if self.selected_image_is_bob else self.play_sound(self.symphony)
            self.sound_playing = True

    def draw_end(self):
        chance = random.randint(0, 100)
        if chance < 90:
            bob_images = [self.bob1, self.bob2, self.bob3, self.bob4, self.bob5, self.bob6, self.bob7, self.bob8,
                          self.bob9]
            selected_bob = random.choice(bob_images)
            self.selected_image_is_bob = True  # Track if bob image is selected
            left = (screen.get_width() - selected_bob.get_width()) // 2
            top = (screen.get_height() - selected_bob.get_height()) // 2
            end_rect = pygame.Rect(left, top, screen.get_width(), screen.get_height())
            screen.blit(selected_bob, end_rect)
        else:
            self.selected_image_is_bob = False  # Track if cow image is selected
            left = (screen.get_width() - self.cow1.get_width()) // 2
            top = (screen.get_height() - self.cow1.get_height()) // 2
            end_rect = pygame.Rect(left, top, screen.get_width(), screen.get_height())
            screen.blit(self.cow1, end_rect)
